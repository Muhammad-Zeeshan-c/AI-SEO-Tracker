...
export async function scrapUrl(url){
    ...
    try{
        //creating browser base session 
        const sessions=await bb.sessions.create({browserSettings:{blockAds:true}})
            browser=await chromium.connectOverCDP(sessions.connectUrl)
            ...
            //Extract all SEO releated data from the rendered page
            const scrapedData=await page.evaluate(()=>{
                ...
                const wordCount=bodyText.split(/\s/).filter(f=>f.length>0).length;
                ...
                return {
                    ...
                    images:{total:allImages.length,missingAlt,withAlt:allImages.length-missingAlt},wordCount,
                    pageSize,
        ...
        console.error('Scrapper playright session failed : ',err)
        ...
        if(browser){
            try{
                ...
                console.error('Scrapper browser connection closing failed',error)
            }
        }
    }
}
